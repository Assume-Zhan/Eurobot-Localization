...
recvVCP1002(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_VCP1002_VOLTAGEINPUT_110:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvTMP1200_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_300:
		...
		case VINT_PACKET_TYPE_RESISTANCEINPUT_RESISTANCECHANGE:
			...
			if (len > 4) {
				if (resistance > 0)
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Resistance is too high to be accurately measured");
				else
					return PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Check polarity of connection");
			}
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", resistance));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_BADCONNECTION:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Bad Connection"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_TMP1200_TEMPERATURESENSOR_RTD_300:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			if (len > 4) {
				if(temperature > 0)
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Temperature is too high to be accurately measured");
				else
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Temperature is too low to be accurately measured");
			}
			temperature = round_double(temperature, 3);
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", temperature));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_SATURATION:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Temperature is unknown"));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_BADCONNECTION:
			if(buf[0])
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Check polarity of connection"));
			else
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: RTD is likely disconnected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	default:
...
recvSTC1003_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1003_STEPPER_200:
		...
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			if(buf[0])
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADPOWER,	"Supply Voltage above expected range"));
			else
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADPOWER, "Supply Voltage below expected range. Possible overcurrent condition."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_STC1003_VOLTAGEINPUT_200:
...
recvSTC1005_0(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1005_STEPPER_100:
		...
		case VINT_PACKET_TYPE_STEPPER_FAULT_ERROR:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAULT, "Fault condition detected. Motor is disengaged and position is lost. Re-engage and re-initialize your system."));
		case VINT_PACKET_TYPE_STEPPER_OVERTEMPERATURE:
			PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAULT, "Fault condition detected. Motor is disengaged and position is lost. Re-engage and re-initialize your system.");
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OVERTEMP, "Overtemperature Condition Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_STC1005_VOLTAGEINPUT_100:
...
recvSTC1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1001_STEPPER_100:
		...
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADPOWER,
...
recvSAF1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_SAF1000_POWERGUARD_100:
		...
		case VINT_PACKET_TYPE_POWERGUARD_OVERVOLTAGE:
			if (buf[0])
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OVERVOLTAGE, "Overvoltage Condition Detected."));
...
recvMOT1101(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (fireSaturation == 0) {
				...
				if (fireSaturation & 0x01)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_SATURATION, "Accelerometer Saturation Detected.");
				if (fireSaturation & 0x02)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_SATURATION, "Gyroscope Saturation Detected.");
				if (fireSaturation & 0x04)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_SATURATION, "Magnetometer Saturation Detected.");
				else
	...
	case PHIDCHUID_MOT1101_ACCELEROMETER_100:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			break;
		case VINT_PACKET_TYPE_AXES_SATURATION:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_SATURATION, "Acceleration Saturation"));
			break;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
sendMOT0110_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_MOT0110_SPATIAL_100_VINT:
		...
		case BP_SETHEATINGENABLED:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SPATIAL_SET_HEATING, buf, 1));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvMOT0110_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT0110_SPATIAL_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (len > 40) {
				if (buf[40] & 0x20)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more spatial readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_MAGNETOMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x04)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more magnetometer readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_GYROSCOPE_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXESUPDATE:
			if (len > 16) {
				if (buf[16] & 0x02)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more gyroscope readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_ACCELEROMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x01)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more accelerometer readings is out or range");
			}
...
recvMOT0100_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT0100_ACCELEROMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x01)
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more accelerometer readings is out or range");
			}
			accel[0] = unpackfloat(buf);
			accel[1] = unpackfloat(buf + 4);
			accel[2] = unpackfloat(buf + 8);
...
recvDST1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DST1000_DISTANCESENSOR_100:
		...
		case VINT_PACKET_TYPE_DISTANCESENSOR_SATURATION:
			...
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Sensor value is ouside the valid range for this sensor."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvDCC1100(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DCC1100_BLDCMOTOR_100:
		...
		case VINT_PACKET_TYPE_BLDCMOTOR_STALLDETECTED:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_MOTORSTALL, "Dangerous motor stall detected. Velocity has been reduced in order to prevent damage."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_DCC1100_BLDCMOTOR_120:
	...
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_100:
		...
		case VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_MOTORSTATUSUPDATE:
			...
			return(bridgeSendToChannel(ch, BP_DUTYCYCLECHANGE, 1, "%g", dutyCycle));
		case VINT_PACKET_TYPE_BLDCMOTOR_STALLDETECTED:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_MOTORSTALL, "Dangerous motor stall detected. Motor has been disengaged in order to prevent damage."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_120:
...
recvOUT1002(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGEOUTPUT_VOLTAGEERROR:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_VOLTAGEERROR, "Voltage Error Detected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_110:
...
recvCURLOOP(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_CURLOOP_CURRENTOUTPUT_100:
		...
		case VINT_PACKET_TYPE_CURRENTOUTPUT_CURRENTERROR:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADPOWER, "Current Error Detected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
